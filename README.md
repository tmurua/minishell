...
**Partner A (Parser)** and **Partner B (Executor)** should:

- Communicate, communicate, communicate. Daily meetings at 3pm.
- Use version control systems, i.e. Github, to manage code collaboratively.
...
- **Average Duration**: Typically, student pairs complete the minishell project in **3 to 7 weeks**.
...
1. **Main Loop**: Core loop for prompt display and user input.		(Shared, Partner A and Partner B)
2. **Input Handling**: Reading and preliminary processing of input.	(Partner A, "Parser")
3. **Lexer/Tokenizer**: Breaking input into tokens.					(Partner A, "Parser")
4. **Parser**: Constructing command structures from tokens.			(Partner A, "Parser")
5. **Executor**: Executing commands, managing built-ins and external commands.	(Partner B, "Executor")
6. **Built-in Commands**: Implementing required built-in functions.	(Shared, Partner A handles echo, cd, pwd; Partner B handles export, unset, env, exit)
7. **Environment Management**: Handling environment variables and expansions.	(Partner A, "Parser/Expander")
8. **Redirections**: Managing input/output redirections.			(Partner B, "Executor")
9. **Pipes**: Handling pipes between commands.						(Partner B, "Executor")
10. **Signal Handling**: Managing signals (`ctrl-C`, `ctrl-D`, `ctrl-\`).		(Partner A, "Parser")
11. **History Management**: Implementing command history using `readline`.
12. **Utilities**: Helper functions and common utilities.			(Shared, Integration)
13. **Error Handling**: Providing user-friendly error messages and handling exit statuses.	(Shared, Integration)
14. **Memory Management**: Ensuring all allocated memory is properly freed.	(Shared, Integration)
...
> Note: The number of modules often reflects the team's preference for code organization and modularity. More modules can enhance readability and maintainability.
>
...
### Partner A (Parser) Responsibilities
...
### Partner B (Executor) Responsibilities
